...
def exercise04(more_temperatures, iot_sensor_points, a, b, c, d, e):
    ...
    samples = []
    copy_of_samples = []
    for value in more_temperatures:
        temperatures.append(value)    
    for value in (iot_sensor_points):
        temperatures.append(iot_sensor_points[value])
    ...
    copy_of_samples
    samples.sort() 
    samples#ascending order
    print("temperatures", temperatures)
    print("copy_of_samples", copy_of_samples)
...
def exercise06(n):
    ...
    print(length_n, sum_n, average_n)
    # ------ Place code above here /\ /\ /\ ------
...
def display_menu(menu):
	if type(menu) is tuple:
		numberOfElements=len(menu)
		choice=0
		temp=0
		while temp !=(numberOfElements+1):
			for i in range(0,numberOfElements):
				print(str(i+1)+". "+menu[i])
			print (str(numberOfElements+1)+". Exit")
			temp = input ("Please make a choice: ")
			if temp=="1":
				choice=1
				buyBitcoin()
			elif temp=="2":
				...
				print("Wrong choice")
		return choice,numberOfElements
	else:
		return -1,len(menu)
...
def exercise10(sentence):
    ...
    reversed=sentence[::-1]
    reversed=''.join(ch.lower() if ch.isupper() else ch.upper() for ch in reversed)
